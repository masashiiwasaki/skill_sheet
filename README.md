...
|2025年8月 – 2026年8月 |フリーランスとして活動開始（個人事業主） |業務委託契約とし複数社契約 |

## 職務経歴

### 2025/11 - 2026/08: 株式会社CareFran

職務: Webアプリケーションエンジニア

ケアマネジャー向け介護支援SaaSを開発。AI文書生成で開発・運用。自社のケアマネジャーのヒアリングを行いながらAI生成の改善や新規機能開発

#### 言語/ツール
- 言語: TypeScript(version: 6.0.2), JavaScript, SQL, Python
- フレームワーク: Next.js(version: 16.2.11), React.js(version: 19.2.8), Hono, LangChain / LangGraph, Tailwind CSS(version: 4.2.1)
- ランタイム: Node.js(version: 24.15.0), Bun(version: 1.3.14)
- ツール類: Notion, GitHub, Playwright, Vitest, Biome, Terraform, lambroll, Sentry, Slack, Docker, mise

#### 機種・OS
- データベース: PostgreSQL（Supabase）
- Webサーバ: AWS ECS Fargate / AWS Lambda（OS: Linux、リージョン: ap-northeast-1）
- ミドルウェア: Docker, AWS ALB

#### 成果
自社内にケアマネジャー(サービス利用者)が実際にサービスを使いながら介護事業所を運営しているので、ヒアリング会を企画しながら改善を行った。患者さんからのヒアリング音声データを元に介護計画書、カルテを作る機能であったが、複数の資料を作成するのに多大な時間がかかってたが三分の一ほどに短縮できた。またプロンプトの改善でヒアリング内容の精度向上にも貢献した。

#### 学んだ事
実際の利用者がみじかにいる環境で開発できたので、ネットの情報だけでは難しかった要件定義などがより深く出来、改善対応も素早く出来た。競合がいるアプリケーションの中でかなり速いスピードで改善を行い使いやすさで選んでもらえたのは大きかった。
